...
`<8>` = WiFi information from Pico to ESP<br>           `$<8>,<SSID (string)>,<password (string)>,<CRC>;`<br>
`<9>` = Send server ip/domain + (port) to ESP<br>       `$<9>,<IP/domain (string)>,<port (int)>,<CRC>;` #if port = 0, port is unused<br>
`<10>` = Api token from Pico to ESP<br>                 `$<10>,<Token (string)>,<CRC>;`<br>
