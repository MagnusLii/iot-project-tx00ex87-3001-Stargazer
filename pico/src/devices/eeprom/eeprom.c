...
static inline bool eeprom_write_cycle_check() {
    if (absolute_time_diff_us(write_init_time, get_absolute_time()) > write_cycle_max) { return true; }
    return false;
}
...
static inline void eeprom_write_cycle_block() {
    if (!eeprom_write_cycle_check()) { sleep_until(delayed_by_us(write_init_time, write_cycle_max)); }
}
...
/**
 * Writes the specified EEPROM address using the I2C interface after ensuring the EEPROM write cycle duration is within
 * limits.
 *
...
/**
 * Writes a byte of data to the specified EEPROM address using the I2C interface after ensuring the EEPROM write cycle
...
/**
 * Writes a page of data to the specified EEPROM address using the I2C interface after ensuring the EEPROM write cycle
 * duration is within limits.
 *
