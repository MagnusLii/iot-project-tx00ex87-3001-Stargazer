...
bool TLSWrapper::connect(const char *host, const char *port, const char *root_cert) {
    ...
    if (strlen(root_cert) == 0) {
        DEBUG("Invalid arguments: root_cert is empty");
        return false;
    }

    DEBUG("Free heap before TLS connection: %d", esp_get_free_heap_size());
    DEBUG("Smallest free heap before TLS connection: %d", esp_get_minimum_free_heap_size());
    multi_heap_info_t info;
    heap_caps_get_info(&info, MALLOC_CAP_8BIT);
    DEBUG("Largest free block: %d", info.largest_free_block);
    ...
    if (ret != 0) {
        char error_buf[100];
        mbedtls_strerror(ret, error_buf, sizeof(error_buf));
        DEBUG("Failed to parse CA certificate, error code: ", ret, " - ", error_buf);
        return false;
    }
